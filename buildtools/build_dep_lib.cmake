...
function(_bd_mirror name repo_root out)
    ...
    set(lock "${repo_root}/prebuilt.lock")
    if(EXISTS "${lock}")
        file(STRINGS "${lock}" _lines REGEX "^${name} ")
        if(NOT _lines)
            # source-delivery deps have no lock lines; any release carries all sources
            file(STRINGS "${lock}" _lines LIMIT_COUNT 1)
        endif()
        if(_lines)
            list(GET _lines 0 _l)
            string(REPLACE " " ";" _l "${_l}")
            list(LENGTH _l _n)
            if(_n EQUAL 7)
                list(GET _l 6 _rel)
                set(${out} "https://github.com/kryksyh/muse_deps_private/releases/download/${_rel}" PARENT_SCOPE)
            endif()
        endif()
    endif()
endfunction()
